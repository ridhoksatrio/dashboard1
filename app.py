...
strats = {
    'champions': {
        'name': '🏆 Champions',
        'grad': 'linear-gradient(135deg,#FFD700,#FFA500, #FF8C00)',
        'color': '#FFD700',
        'priority': 'CRITICAL',
        'strategy': 'VIP Platinum',
        'tactics': ['💎 Exclusive Early Access', '🎁 Premium Gifts', '📞 24/7 Manager', '🌟 VIP Events', '✨ Celebrations'],
        'kpis': ['Retention>95%', 'Upsell>40%', 'Referral>30%'],
        'budget': '30%',
        'roi': '500%'
    },
    'loyal': {
        'name': '💎 Loyal',
        'grad': 'linear-gradient(135deg,#667eea,#764ba2,#5a52a3)',
        'color': '#667eea',
        'priority': 'HIGH',
        'strategy': 'Loyalty Boost',
        'tactics': ['🎯 Tiered Rewards', '📱 App Benefits', '🎉 Birthday Offers', '💝 Referral Bonus', '🔔 Flash Access'],
        'kpis': ['Retention>85%', 'Frequency+20%', 'NPS>8'],
        'budget': '25%',
        'roi': '380%'
    },
    'big': {
        'name': '💰 Big Spenders',
        'grad': 'linear-gradient(135deg,#f093fb,#f5576c,#d2368d)',
        'color': '#f093fb',
        'priority': 'CRITICAL',
        'strategy': 'Value Max',
        'tactics': ['💳 Flex Terms', '🎁 Luxury Gifts', '🚚 Free Express', '📦 Custom Bundles', '🌟 Concierge'],
        'kpis': ['AOV+15%', 'Retention>90%', 'Sat>4.8/5'],
        'budget': '20%',
        'roi': '420%'
    },
    'dormant': {
        'name': '😴 Dormant',
        'grad': 'linear-gradient(135deg,#ff6b6b,#ee5a6f,#c44569)',
        'color': '#ff6b6b',
        'priority': 'URGENT',
        'strategy': 'Win-Back',
        'tactics': ['🎁 25-30% Off', '📧 Multi-Channel', '🎯 Retargeting', '💬 Personal Call', '⏰ Urgency'],
        'kpis': ['Winback>25%', 'Response>15%', 'ROI>200%'],
        'budget': '15%',
        'roi': '250%'
    },
    'potential': {
        'name': '🌱 Potential',
        'grad': 'linear-gradient(135deg,#11998e,#38ef7d,#00b09b)',
        'color': '#11998e',
        'priority': 'MEDIUM',
        'strategy': 'Fast Convert',
        'tactics': ['🎓 Education', '🎁 15% 2nd Buy', '💌 Welcome Flow', '📚 Tutorials', '🎯 Cross-Sell'],
        'kpis': ['Convert>35%', '2nd<30d', 'LTV+25%'],
        'budget': '5%',
        'roi': '180%'
    },
    'standard': {
        'name': '📊 Standard',
        'grad': 'linear-gradient(135deg,#89f7fe,#66a6ff,#4a6fff)',
        'color': '#89f7fe',
        'priority': 'MEDIUM',
        'strategy': 'Steady Engage',
        'tactics': ['📧 Newsletters', '🎯 Seasonal', '💌 AI Recs', '🎁 Surprises', '📱 Community'],
        'kpis': ['Engage>40%', 'Stable', 'Sat>3.5/5'],
        'budget': '5%',
        'roi': '150%'
    }
}
...
def main():
    ...
    if monetary_filter and 'Monetary' in filtered_df.columns:
        filtered_df = filtered_df[
            (filtered_df['Monetary'] >= monetary_filter[0]) & 
            (filtered_df['Monetary'] <= monetary_filter[1])
        ]
    
    if frequency_filter and 'Frequency' in filtered_df.columns:
        filtered_df = filtered_df[
            (filtered_df['Frequency'] >= frequency_filter[0]) & 
            (filtered_df['Frequency'] <= frequency_filter[1])
        ]
    
    if recency_filter and 'Recency' in filtered_df.columns:
        filtered_df = filtered_df[
        ...
        if len(filtered_df) > 0:
            ...
            if 'Cluster_Label' in filtered_df.columns:
                ...
                if 'Recency' in filtered_df.columns:
                    ...
                    least_recent_segment = recency_by_segment.idxmax() if not recency_by_segment.empty else "N/A"
                ...
                if 'Monetary' in filtered_df.columns:
                    ...
                    dormant_avg_monetary = 0
                    if dormant_clusters and 'Monetary' in filtered_df.columns:
                        dormant_avg = filtered_df[filtered_df['Cluster_KMeans'].isin(dormant_clusters)]['Monetary'].mean()
                        dormant_avg_monetary = dormant_avg if not pd.isna(dormant_avg) else 0
                    
                    if dormant_avg_monetary > 0:
                        dormant_uplift_potential = ((avg_monetary - dormant_avg_monetary) / dormant_avg_monetary * 100)
                ...
                with insight_tab1:
                    ...
                    with mcol1:
                        champion_rev_share = (champion_revenue / total_revenue * 100) if total_revenue > 0 else 0
                        st.metric(
                            "🏆 Champion Revenue Share",
                            f"{champion_rev_share:.1f}%",
                            "+3.2%"
                        )
                    ...
                    with mcol3:
                        dormant_pct = (dormant_count/total_customers*100) if total_customers > 0 else 0
                        st.metric(
                            "😴 Dormant Customers",
                            f"{dormant_count:,}",
                            f"{dormant_pct:.1f}%"
                ...
                with insight_tab2:
                    ...
                    with col1:
                        potential_revenue = dormant_count * avg_monetary if 'avg_monetary' in locals() else 0
                        st.markdown(f"""
                        <div class="insight-card">
                            ...
                            <ul class="insight-list">
                                ...
                                <li>📈 <strong>Revenue Uplift:</strong> ${potential_revenue:,.0f} potential from dormant</li>
                    ...
                    with col2:
                        ...
                        <div class="insight-card">
                            ...
                            <ul class="insight-list">
                                ...
                                <li>🏆 <strong>Champions</strong> contribute <strong>{champion_rev_share:.1f}%</strong> of total revenue</li>
