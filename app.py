...
# Fungsi untuk membuat chart (disesuaikan dari kode Dash asli)
def create_charts(df):
    ...
    f4.update_layout(height=380, margin=dict(t=20, b=20, l=20, r=20))
    
    # Return hanya 4 chart yang dibuat
    return f1, f2, f3, f4
...
def main():
    ...
    # Apply filters
    filtered_df = rfm[
        (rfm['RFM_Score'] >= rfm_filter[0]) & 
        (rfm['RFM_Score'] <= rfm_filter[1])
    ]
    ...
    with tab1:
        if len(filtered_df) > 0:
            # Generate charts based on filtered data - HANYA 4 CHART
            f1, f2, f3, f4 = create_charts(filtered_df)
